...
\section{Processi di Supporto}
	...
	\subsection{Processo di documentazione}
		...
		\subsubsection{Procedure}
			...
			Per ogni documento che il \roleProjectManager{} ritiene utile essere steso dovrà essere generato con appositi tools messi a disposizione. Essi permettono di mantenere un certo ordine e rigore logico.  
				...
				\begin{enumerate}
					\item posizionarsi all'interno della cartella \emph{documenti} presente nel repository\gloss{};
				...
				\begin{enumerate}
					\item l'assegnatario dovrà effettuare l'upload del documento all'interno del branch predisposto presente nel repository\gloss{};
					\item l'assegnatario segnerà come completato il ticket\gloss{} assegnatogli;
					\item il \emph{Verificatore} riceverà automaticamente una mail con la conferma dell'avvenuta conclusione;
					\item il \roleVerifier{} assegnerà un ticket\gloss{} per la risoluzione de problema al redattore del documento;
					\item qualora il documento sia corretto si procederà alla chiusura del ticket\gloss{}, avvisando automaticamente il \roleProjectManager{}, che a sua volta provvederà a contrassegnare il documento come approvato. Altrimenti il \roleVerifier{} assegnerà nuovi ticket al redattore. In seguito il redattore provvederà a eseguire di nuovo tutti i passi del seguente elenco;
				\end{enumerate}
			...
			Ogni membro, a seconda di chi il \roleProjectManager{} riterrà opportuno, potrà andare a redigere il glossario. \\
				...
				Una volta terminata la stesura del \docNameVersionGlo{} l'\roleAdministrator{} provvederà ad eseguire gli script necessari all'inserimento dei termini ambigui seguendo la seguente procedura da terminale:
					\begin{enumerate}
						\item dalla root del repository\gloss{} entrare nella cartella script ed invocare il comando:
							\begin{center}
		...
		\subsubsection{Norme}
			...
			\paragraph{Template}
			Al fine di ottenere dei documenti formattati correttamente si è predisposto un template\gloss{} in \LaTeX{} da seguire scrupolosamente. Esso deve essere utilizzato come linea guida per lo sviluppo dei documenti. Ogni tipologia di documento, prima pagina, indici, contenuti interni, lettere hanno uno stile definito in maniera univoca. Esso ne permette una gestione capillare ed omogenea.\\
			...
			Tutti i documenti devono rispettare le seguenti norme tipografiche, utili per mantenere i documenti omogenei tra loro.
				...
				\begin{itemize}
					...
					\item \textbf{Numeri}: i numeri dovranno rispettare lo standard SI/ISO\gloss{} 31-0 
				...
				\begin{itemize}
					...
					\item \textbf{Date:} ogni data deve essere scritta come indicato dallo standard ISO\gloss{} 8601:\\
						\begin{displaymath}
							AAAA-MM-GG
						\end{displaymath}
						dove:
		...
		Per semplificare ed automatizzare il lavoro sono stati creati alcuni script che verranno lanciati o automaticamente durante una determinata azione sul repository\gloss{} o in maniera manuale dai membri del team quando necessario.
			\paragraph{Script} % (fold)
			\label{par:script_doc}
				\begin{itemize}
					\item \textbf{Generazione struttura documento}: il makefile predisposto per la nuova generazione dei documenti ha il ruolo di effettuare una copia del template\gloss{} di documento ed assegnargli il nome;
	...
	Il processo prevede una prima fase di pianificazione e successivamente di verifica.
		...
		\subsubsection{Attività}
			...
			Il \roleVerifier, durante le attività di controllo, nel caso si riscontrassero anomalie o discordanze normative avrà il compito di notificarle all'assegnatario del task\gloss{} sul quale sta effettuando la verifica.\\
		...
		\label{ssub:procedure}
			...
			L'assegnazione delle anomalie riscontrate da parte del \roleVerifier{} deve avvenire tramite l'uso della piattaforma Asana\gloss{}.\\
			Esistono due tipi di sezioni sulle quali effettuare la segnalazione:
			\begin{itemize}
				\item \textbf{Bug\gloss{} Tracking Document}: sezione dedicata alle anomalie riguardante la documentazione; 
				\item \textbf{Bug\gloss{} Tracking Source}: sezione dedicata alle anomalie riguardante il codice sorgente del software.
			\end{itemize}
			\noindent
			\`E necessario fare uso di uno dei due template\gloss{} messi a disposizione, presenti all'interno delle viste sopraelencate.\\
			...
			\begin{enumerate}
				\item definizione del nuovo titolo, contenente nella parte iniziale la dicitura {[}bug\gloss{}{]};
			...
			\end{enumerate}
			\noindent
			Il sistema automaticamente provvederà alla notifica del nuovo task\gloss{} attraverso l'invio di una mail.
			\begin{figure}[htbp]
				\centering
