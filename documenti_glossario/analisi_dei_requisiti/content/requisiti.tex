...
A seguire sono riportati tutti i requisiti individuati dal gruppo. Essi sono stati ricavati dal capitolato proposto, dai casi d'uso, da necessità interne o in seguito all' incontro col proponente\gloss{}. Ogni requisito è identificato da un codice univoco a seconda dell'importanza, del tipo e della gerarchia a cui appartiene.
...
\begin{center}
	...
	\begin{longtable}{| p{2cm} | p{8cm} | p{2cm} |}
		...
		\hline
		R0F1.5  &  Il sistema avverte l'utente in fase di compilazione della form\gloss{} nel caso i dati inseriti in quel momento non risultino conformi alle norme del sistema  &  Interno \\
		\hline
		...
		\hline
		R0F1.8  &  Nel caso la registrazione sia andata a buon fine, il sistema aggiunge i dati dell'utente nel rispettivo database\gloss{}  &  Interno \\
		\hline
		...
		\hline
		R0F2  &  L'utente non autenticato può effettuare il login tramite l'apposto form\gloss{} di login  &  UC1.1 \\
		\hline
		...
		\hline
		R0F3.2.2  &  Se l'utente seleziona il pulsante di visualizzazione delle statistiche deve aprirsi una finestra che mostra il numero di View\gloss{} attive, il numero di Recipe\gloss{} disponibili e il numero di accessi negli ultimi 30 giorni &  UC1.2.3 \\
		\hline
		R0F3.3  &  Nel menù delle informazioni personali è presente un pulsante per modificare la propria password &  UC1.5 \\
		\hline
		R0F3.3.1  &  Se l'utente seleziona il pulsante di cambio della password, si deve aprire una finestra con il form\gloss{} per l'inserimento di una nuova password e della password di conferma. Viene richiesto anche l'inserimento della vecchia password. &  UC1.5 \newline UC1.5.1 \newline UC1.5.2 \newline UC1.5.3 \\
		\hline
		...
		\hline
		R0F3.3.3  &  L'utente può cliccare il tasto di conferma se ha riempito il form\gloss{}  &  UC1.5.4 \\
		\hline
		...
		\hline
		R0F3.3.3.3  &  Una volta premuto il tasto di conferma il sistema aggiorna la password nel Database\gloss{}  &  Interno \\
		\hline
		...
		\hline
		R0F5  &  Nella home screen dell'utente autenticato sono mostrare tutte le View\gloss{} che l'utente ha creato aggiornate al momento del caricamento della pagina &  UC1.3 \newline UC1.3.1 \\
		\hline
		R0F5.1  &  Il sistema ad ogni caricamento della pagina deve recuperare i dati relativi alle View\gloss{} dell'utente e generare i grafici relativi  &  Interno \\
		\hline
		R1F5.2  &  Nella home screen è disponibile un elenco delle View\gloss{} attive dell'utente  &  UC1.3.1.1 \\
		\hline
		R1F5.2.1  &  Cliccando su una View\gloss{} dall'elenco di viene portati al grafico relativo  &  Interno \\
		\hline
		R1F5.2.2  &  Accanto ad ogni voce dell'elenco è disponibile il tasto modifica  &  UC1.3.4 \newline UC1.3.4.1 \newline UC1.3.4.2 \newline UC1.3.6.1 \\
		\hline
		R1F5.2.2.1  &  Cliccando sul tasto modifica si viene reindirizzati alla pagina di modifica della View\gloss{}  &  Interno \\
		\hline
		R1F6  &  Nella pagina di modifica della View\gloss{} è presente il grafico relativo alla View selezionata e un form\gloss{} per modificare i dati  &  Interno \\
		\hline
		R1F6.1  &  Il sistema deve recuperare i dati relativi alla View\gloss{} selezionata e generare il grafico e riempire la form\gloss{} con i dati preesistenti  &  Interno \\
		\hline
		R1F6.2  &  L'utente deve poter inserire nuovi dati oppure lasciare quelli preesistenti  &  UC1.3.4.3 \\
		\hline
		R1F6.2.1  &  Eventuali errori sui dati inseriti vengono segnalati dal sistema in fase di compilazione del form\gloss{}  &  Interno \\
		\hline
		...
		\hline
		R1F6.3.1.1  &  Se la modifica è avvenuta con successo il sistema memorizza i nuovi dati nel database\gloss{} sovrascrivendo i vecchi  &  Interno \\
		\hline
		R1F6.3.2  &  Se c++\gloss{}'è un errore sui dati o c'è un problema di connessione al server viene visualizzato un messaggio di errore e si viene riportati alla form\gloss{} di modifica  &  UC1.3.5 \\
		\hline
		R1F6.4  &  Nella pagina di modifica è presente un tasto per eliminare la View\gloss{}  &  UC1.3.6 \newline UC1.3.6.2 \\
		\hline
		...
		\hline
		R1F6.4.2  &  Se l'eliminazione viene confermata la View\gloss{} e i relativi dati vengono eliminati dal database\gloss{}  &  Interno \\
		\hline
		R1F6.4.3  &  Una volta eliminata una View\gloss{} si viene reindirizzati alla home screen  &  Interno \\
		\hline
		R0F7  &  Nella home screen è presente un tasto per creare una nuova View\gloss{}  &  UC1.3.2 \newline UC1.3.2.1 \\
		\hline
		R0F7.1  &  Nella pagina di creazione della View\gloss{} è presente un form\gloss{} per inserire i dati della View da generare  &  UC1.3.2.2 \\
		\hline
		R0F7.1.1  &  Eventuali errori sui dati inseriti vengono segnalati dal sistema in fase di compilazione del form\gloss{}  &  Interno \\
		\hline
		...
		\hline
		R0F7.3.1  &  Se la modifica è avvenuta con successo il sistema memorizza i nuovi dati nel database\gloss{}  &  Interno \\
		\hline
		R0F7.3.2  &  Se c++\gloss{}'è un errore sui dati o c'è un problema di connessione al server viene visualizzato un messaggio di errore e si viene riportati alla form\gloss{} di inserimento  &  UC1.3.3 \\
		\hline
		...
		\hline
		R0F8.1  &  Un amministratore può visualizzare tutte le Recipe\gloss{} memorizzate nel sistema selezionando l'apposito pulsante  &  UC1.4.1 \\
		\hline
		R0F8.2  &  Un amministratore può accedere al pannello per inserire una nuova Recipe\gloss{}  &  UC1.4.2 \newline UC1.4.2.1 \newline UC1.4.2.2  \\
		\hline
		R0F8.2.1  &  Il sistema richiede l'inserimento di determinati parametri per individuare il contesto dei dati richiesti dalla Recipe\gloss{}  &  Interno \newline UC1.4.2.3 \\
		\hline
		R0F8.2.2  &  Il sistema richiede la conferma dei dati inseriti prima di creare la nuova Recepie  &  UC1.4.2.4 \\
		\hline
		R0F8.3  &  Il sistema visualizza un messaggio di errore contenente i parametri inseriti non corretti oppure il messaggio di una eccezione lasciato in fase di creazione della Recipe\gloss{}  &  UC1.4.3 \\
		\hline
		R0F8.3.1  &  Nel caso venga confermata  una nuova Recipe\gloss{} corretta, il sistema salva i suoi parametri nel database\gloss{}  e riporta l'amministratore all'elenco delle Recipe  &  Interno \\
		\hline
		R0F8.4  &  L'utente amministratore può eliminare una Recipe\gloss{} memorizzata nel sistema tramite il relativo pulsante nella schermata di elenco delle Recipe  &  UC1.4.4 \newline UC1.4.4.1 \newline UC1.4.4.2 \\
		\hline
		R0F8.4.1  &  L'utente amministratore deve confermare l'eliminazione della Recipe\gloss{} una volta premuto l'apposito pulsate  &  UC1.4.4.3 \\
		\hline
		R0F8.4.2  &  Una volta confermata l'eliminazione di una Recipe\gloss{}, il sistema provvede all'eliminazione di tutte le informazioni legate ad essa presenti nel database\gloss{}  &  Interno \\
		\hline
		R0F8.4.3  &  Una volta confermata l'eliminazione di una Recipe\gloss{}, il sistema provvede ad eliminare tutti i dati grezzi presenti nel database\gloss{} legati a tale Recipe  &  Interno \\
		\hline
		R2F8.5  &  Dal pannello di amministrazione è possibile visualizzare feedback e statistiche sugli utenti in relazione ai dati delle Recipe\gloss{} utilizzati  &  Interno \\
		\hline
		...
		\hline
		R2F9.2.1  &  Premendo conferma il ruolo e i permessi dell'utente vengono modificati sul database\gloss{}  &  UC1.7.2.4 \\
		\hline
		...
		\hline
		R1F9.3.1.3  &  Se c++\gloss{}'è stato un errore appare un messaggio d'errore e si è riportati alla pagina di modifica di un utente  & UC1.7.4\\
		\hline
...
\begin{center}
	...
	\begin{longtable}{| p{2cm} | p{8cm} | p{2cm} |}
		...
		R1P1  &  Una volta autenticato l'utente deve poter visualizzare le proprie View\gloss{} entro 10 secondi salvo problemi di connesione &  Interno \\
		\hline
		R1P1.1  &  Il recupero dei dati dal database\gloss{} deve impiegare al più 7 secondi  &  Interno \\
		\hline
...
\begin{center}
	...
	\begin{longtable}{| p{2cm} | p{8cm} | p{2cm} |}
		...
		\hline
		R1V1.1  &  Il sistema sfrutterà Google App Engine\gloss{}  &  Capitolato \newline Verbale 2015/01/14 \\
		\hline
		R1V1.2  &  Il sistema sfrutterà Google Cloud Datastore  &  Capitolato \newline Verbale 2015/01/14 \\
		\hline
		R1V1.2.1  &  Il database\gloss{} sarà di tipo non relazione composto da documenti in stile JSON\gloss{}, stesso formato dei dati in entrata  &  Interno \newline Verbale 2015/01/14 \\
		\hline
