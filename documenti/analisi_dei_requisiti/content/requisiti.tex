...
\begin{center}
	...
	\begin{longtable}{| p{2.5cm} | p{8cm} | p{2cm} |}
		...
		\hline
		ROF6.2  & L'utente ha la possibilità di aggiungere qualsiasi View a quelle preferite premendo sull'apposito pulsante vicino alla View che desidera aggiunngere.  & UC1.11.2 \newline Interno \\
		\hline
		...
		\hline
		ROF8.1.1.1  &  Viene richiesto l'inserimento del titolo della Recipe.  &  UC1.4.1.2.1 \\
		\hline
		ROF8.1.1.2  &  Viene richiesto l'inserimento di una descrizione della Recipe.  &  UC1.4.1.2.2 \\
		\hline
		ROF8.1.1.3  &  L'utente amministratore può aggiungere un numero qualsiasi di metriche alla Recipe.  &  UC1.4.1.2.3 \\
		\hline
		...
		\hline
		ROF8.2.1  &  Il sistema richiede la conferma dei dati inseriti prima di creare la nuova Recipe.  &  UC1.4.1.3 \\
		\hline
		ROF8.2.2  &  Nel caso venga confermata una Recipe che non contiene almeno una metrica per categoria o l'utente non ha inserito dei parametri corrispondenti a quelli specificati nel requisito ROF8.1.1.4 il sistema visualizza un messaggio di errore contenente i parametri inseriti non corretti oppure il messaggio di una eccezione lanciato in fase di creazione della Recipe.  &  UC1.4.2 \\
		\hline
		ROF8.2.3  &  Nel caso venga confermata  una nuova Recipe corretta, il sistema salva i suoi parametri nel database e riporta l'amministratore all'elenco delle Recipe  &  Interno \\
		\hline
