...
\begin{center}
	...
	\begin{longtable}{| p{2cm} | p{8cm} | p{2cm} |}
		...
		\hline
		RDF10.1  &  Il sistema è un grado di gestire una richiesta di un token di accesso per utilizzare i servizi REST se l'utente è autenticato  & UC2.1 \newline UC2.1.1 \\
		\hline
		RDF10.1.1  &  [TO CHANGE] Il sistema restituisce un errore se il sistema ha avuto problemi nel generare il token & UC2.1.2 \\
		\hline
		RDF10.1.2  &  Il sistema genera e restituisce un token di accesso se l'utente è autenticato  & UC2.1.3 \\
		\hline
		RDF10.2  &  I token di accesso sono stringhe alfanumeriche di 200 caratteri  & Interno \\
		\hline
		RDF10.3  &  L'utente può richiedere che un token valido venga considerato non valido  & UC2.3 \\
		\hline
		RDF10.3.1  &  Il sistema è in grado di gestire una richiesta di chiusura sessione contenente un token di accesso valido rendendolo non valido & UC2.3 \newline UC2.3.1 \\
		\hline
		RDF10.3.2  &  [TO CHANGE] Il sistema restituisce un errore se non è riuscito a invalidare il token & UC2.3.2 \\
		\hline
		RDF10.3.3  &  Il sistema restituisce una conferma dell'avvenuta invalidazione del token & UC2.3.3 \\
		\hline
		RDF10.4  &  Il sistema è in grado di gestire una richiesta che richiede i dati di una View contenente un token di accesso  &  UC2.2 \newline UC2.2.1 \\
		\hline
		RDF10.4.1  &  Il sistema restituisce i dati richiesti se il token utilizzato è valido  &  UC2.2.3 \\
		\hline
		RDF10.4.2  &  Il sistema restituisce un errore se il token utilizzato è non valido  &  UC2.2.2 \\
		\hline
