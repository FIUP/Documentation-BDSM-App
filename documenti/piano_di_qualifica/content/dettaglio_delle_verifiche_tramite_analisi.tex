...
\section{Dettaglio delle verifiche tramite analisi}
	...
	\subsection{Progettazione di dettaglio e codifica dei requisiti obbligatori}
		...
		\subsubsection{Soddisfacimento metriche}
			Questa sezione traccia i risultati ricavati applicando le metriche relative al codice descritte nella sezione 2.8.3 di questo documento.\\
			\paragraph{Soddisfacimento metriche componenti}
				Nella tabella 11 viene riportata la percentuale di copertura del codice dei test in relazione alle componenti \texttt{client} e \texttt{server}.
				\begin{table}[!ht]
					\begin{center}
						\begin{tabularx}{0.9\textwidth}{|l|l|X|}
							\hline
							\textbf{Componente} & \textbf{Indice di copertura} & \textbf{Esito}\\
							\hline
							\texttt{src::client} & ?? & ??\\
							\hline
							\texttt{src::server} & ?? & ??\\
						...
						\end{tabularx}
					\end{center}
					\caption{Esiti indice di copertura}
				\end{table}
			\paragraph{Soddisfacimento metriche classi}
				Nella tabella 12 vengono riportati per ogni classe il numero di attributi e il rapporto tra linee di commento e quelle di codice.
