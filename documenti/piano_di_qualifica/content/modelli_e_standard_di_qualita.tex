...
obiettivi specifici
	...
	\label{sub:ciclo_pdca}
	Il ciclo PDCA, noto anche come ciclo di Deming, è un metodo di gestione iterativo a quattro fasi per il controllo e il miglioramento continuo dei processi. Tutti i processi saranno gestiti facendo riferimento a tale modello che permette un incremento della qualità dei processi ad ogni ciclo d'iterazione.\\
	Le quattro fasi che lo compongono sono:
			\begin{itemize}
				\item \textbf{Plan:} vengono stabiliti gli obiettivi e i processi necessari a raggiungere la qualità attesa, più in particolare, la fase sarà caratterizzata dalla valutazione della situazione attuale, dalla comprensione della natura del problema da risolvere e dalla ricerca ed analisi delle possibili soluzioni al problema;
				\item \textbf{Do:} fase composta dall'attuazione del piano applicando le soluzioni al problema trovate nella fase precedente;
				\item \textbf{Check:} si analizzano i risultati ottenuti nella fase precedente. Si confrontano i nuovi dati ottenuti con quelli attesi per verificare che quanto fatto sia corretto. Si producono grafici e tabelle per avere un riassunto chiaro su quanto rilevato. In caso di obiettivo raggiunto si potrà procedere passando all'ultima fase del ciclo PDCA, altrimenti, si ripeterà nuovamente il ciclo PDCA analizzando le cause che non hanno permesso di raggiungere l'obiettivo stabilito;
				\item \textbf{Act:} la soluzione al problema individuata e considerata adeguata nella fase precedente viene standardizzata e i membri del gruppo verranno informati. Terminata questa fase di procederà con una nuova iterazione del ciclo partendo dalla prima fase.
			\end{itemize}
			\begin{figure}[h]
				\centering
