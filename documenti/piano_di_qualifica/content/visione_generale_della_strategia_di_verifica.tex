...
\section{Visione strategia di verifica}
	...
	Le risorse necessarie alla verifica della qualità dei processi e del prodotto sono:
		\begin{itemize}
  			...
  			\item \textbf{Risorse hardware:} \textnormal{sono necessari computer per scrivere documentazioni e creare il prodotto software e stabili in cui poter lavorare al progetto.}
		\end{itemize}
...
lista di controllo definita gradualmente, sia grazie all'esperienza personale, sia attraverso l'arricchimento derivante dall'attività di Walkthrough. Ogni difetto riscontrato verrà corretto e documentato con dei rapporti sulle attività svolte. La lista di controllo inizialmente non sarà abbastanza ampia da garantire l'affidabilità dell'elemento analizzato, per questo almeno nelle prime fasi verrà affiancata al Walkthrough.
		...
		Applicabile solamente al prodotto software e permette di verificarlo e validarlo attraverso l'esecuzione di test. Questi test per essere significativi devono essere ripetibili, ossia dato lo stesso input sul medesimo ambiente di esecuzione, forniscono lo stesso risultato. In questo modo è più facile riscontrare l'origine di eventuali anomalie.
			\begin{itemize}
