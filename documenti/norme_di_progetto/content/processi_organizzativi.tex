...
\section{Processi Organizzativi}
	...
	\subsection{Gestione dei Processi}
		\subsubsection{Attività}
			\paragraph{Gestione delle Comunicazioni}
				...
				Quelle formali avverranno attraverso il sistema di tasking Asana che consente una chat di gruppo al suo interno e che notifica a tutti gli altri membri tramite mail quando qualcuno scrive qualcosa in essa. \\
			...
			\paragraph{Gestione dei task} \label{sec:gestione_dei_task}
			Per pianificare i compiti che i singoli membri dovranno andare a eseguire viene usato l'applicazione web Asana che consente di creare facilmente i task. \\
			...
			Ogni membro del team può proporre dei cambiamenti da apportare. La modifica dovrà essere motivata adeguatamente in modo che il \roleProjectManager{} non debba preoccuparsi di ricercare le fonti che forniscono l'effettiva validità di questo mutamento. \\
			Una volta verificato la necessità o meno del cambiamento, il \roleProjectManager{} approverà o respingerà la proposta fatta.
			...
			Il servizio viene attivato sia in doc\_BDSM\_App sia in src\_BDSM\_App.
				...
				TO DO di collegare i repository presenti con Asana tramite l'API\_Key di uno degli utenti iscritti 
				TO DO (dire dell'account MainMashUp) \\
				TO DO (riferimento al tracciamento delle commit con i task per le quali sono svolte) \\
				TO DO (riferimento alla sezione che segna l'id del task e al suo reperimento) \\	
		...
		\subsubsection{Procedure}
			...
			\paragraph{Procedura per il reperimento dell'id del task su Asana}
			TO DO
				\begin{enumerate}
					\item TO DO
					\item TO DO
				\end{enumerate}
			TO DO (figura illustrativa di dove si trova l'id del task)
			...
			Qualora si trovasse davanti a problemi non previsti dovrà eseguire la seguente sequenza di attività:
				\begin{enumerate}
		...
		\subsubsection{Norme}
			...
			I task che si andranno ad aprire devono contenere le seguente informazioni:
				...
				\end{itemize}
			\noindent
			La figura \ref{fig:template_task_asana} illustra come vengono rappresentate queste informazioni su Asana. Inoltre viene proprio presa da uno screenshot di un template di task, copiabile dal \roleProjectManager{} e modificabile una volta copiato a seconda dell'esigenze riportate in precedenza.
			\begin{figure}[htbp]
				\centering
				\includegraphics[width=14cm]{images/template_task_asana.pdf}
				\caption{Template Task su Asana}
				\label{fig:template_task_asana}
			\end{figure}
			
			\paragraph{Formato delle change request} NOTA: DA SPOSTARE NEL PROCESSO DI SUPPORTO -> Risoluzione di Problemi \\
			Le change request che si andranno ad aprire devono contenere le seguente informazioni:
				...
				\end{itemize}
				La figura \ref{fig:template_change_request_asana} illustra come vengono rappresentate queste informazioni su Asana. Inoltre viene proprio presa da uno screenshot di un template di task, copiabile da qualunque membro desideri proporre un cambiamento e modificabile una volta copiato a seconda dell'esigenze riportate in precedenza.
				\begin{figure}[htbp]
					\centering
					\includegraphics[width=14cm]{images/template_change_request_asana.pdf}
	...
	\subsection{Gestione delle Infrastrutture}
		\subsubsection{Attività}
			...
			\paragraph{Gestione del template messaggio della commit}
		...
		\subsubsection{Procedure}
			...
			\begin{figure}[htbp]
				\centering
				\includegraphics[width=14cm]{images/installazione_git_hook.pdf}
				\caption{Installazione/Aggiornamento dei Git Hooks}
				\label{fig:installazione_git_hook}
			\end{figure}
			
			\paragraph{Installazione e manutenzione messaggio di commit} \label{sec:installazione_msg_commit}
			I membri del team dovranno eseguire questa procedura una volta ricevuta la notifica da parte dell'\roleAdministrator. \\
			...
			\begin{figure}[htbp]
				\centering
				% \includesvg[width=15cm]{images/installazione_tpl_commit_msg.svg}
				\includegraphics[width=14cm]{images/installazione_tpl_commit_msg.pdf}
				\caption{Installazione/Aggiornamento del template del messaggio per la commit}
