...
\section{Processi di Supporto}
	...
	Con il processo di verifica e validazione, che fa parte delle procedure di assicurazione di qualità del prodotto, si vuole introdurre una approfondita analisi del rispetto dei requisiti fissati prima dell'inizio del progetto.\\
	Tale tecnica permette di aumentare il fattore efficienza/efficacia e di ridurre il tempo impiegato nell'analisi.\\
	Con il processo di validazione, in particolare, si vuole determinare se il sistema finale, cosi come è stato costruito, svolge il compito per cui è stato progettato. La parte di validazione può essere svolta dall'acquirente, dal fornitore oppure da una parte indipendente. Per questo progetto verrà svolta internamente al team di sviluppo, utilizzando i test di validazione dettagliatamente tracciati ed elencati nel \docNameVersionPdQ e identificati da una specifica sigla come descritto nel capitolo \ref{subp:validazione} di questo documento.\\
	Per ulteriori informazioni sul Processo di Validazione si veda lo standard ISO/IEC 12220-5. \\
	Il processo di verifica e validazione prevede una prima fase di pianificazione e successivamente una di verifica.
		...
		\subsubsection{Attività}
			...
			Questi script vengono eseguiti manualmente dai verificatori per ciascuna parte del progetto. \\
