	...
	\paragraph{Classi} % (fold)
		...
		\label{subp:server_db_raw_data_tw_rawtwhashtag}
			\begin{itemize}
				...
				\item \textbf{Utilizzo}: la classe viene utilizzata per fornire una descrizione minimale di un hashtag su Twitter. Vengono forniti metodi automatici per il conteggio dei parametri che verranno utilizzati per seguire un trend, in oltre contiene un campo dati che conta i tweet relativi all'hashtag selezionato;
				\item \textbf{Classi ereditate}: server::db::raw\_data::AbsTwRawData
				\item \textbf{Relazioni con altre classi}:
					\begin{itemize}
